...
namespace FFU_Terra_Liberatio {
    public class FFU_TL_Research {
        public static void rebalanceResearch() {
            ModLog.Message($"Updating research times based on their costs.");
            foreach (var rCost in LOOTBAG.researchCosts) {
                if (LOOTBAG.researchTimes.ContainsKey(rCost.Key)) {
                    LOOTBAG.researchTimes[rCost.Key] = rCost.Value / 5000f * 3f;
                }
            }
        }
        public static void dumpForgottenResearch(string dumpFile = "FFU_TL_Forgotten.txt") {
            ...
            foreach (var rEntry in LOOTBAG.researchCosts) {
                var rKey = rEntry.Key;
                var isBlocked = true;
                ...
                if (isBlocked && rModule != Color.Black) {
                    ...
                    $"Type: {FFU_TL_Defs.rMod[rModule.R][rModule.G][rModule.B]?.type!}, " +
                    $"Name: {FFU_TL_Defs.rMod[rModule.R][rModule.G][rModule.B]?.tip?.tip?.Replace("\n", " ")}, " +
                    $"Description: {FFU_TL_Defs.rMod[rModule.R][rModule.G][rModule.B]?.tip?.description?.Replace("\n", " ")}" +
                    $"");
                } else if (isBlocked) {
                    ioDump.WriteLine($"Entry #{rKey}U" +
                    $"{(LOOTBAG.tier.ContainsKey(rKey) ? $", Tier: {LOOTBAG.tier[rKey]}" : "")}" +
            ...
            for (byte r = 0; r < byte.MaxValue; r++) {
                for (byte g = 0; g < byte.MaxValue; g++) {
                    for (byte b = 0; b < byte.MaxValue; b++) {
                        if (FFU_TL_Defs.rMod.ContainsKey(r) && FFU_TL_Defs.rMod[r].ContainsKey(g) && FFU_TL_Defs.rMod[r][g].ContainsKey(b) && FFU_TL_Defs.rMod[r][g][b].rotation == 0) {
                            if (!LOOTBAG.modules.ContainsValue(new Color(r, g, b))) ioDump.WriteLine($"No Entry, " +
                            $"Key: [{r}][{g}][{b}]/({r}, {g}, {b}), " +
                            $"Type: {FFU_TL_Defs.rMod[r][g][b]?.type!}, " +
                            $"Name: {FFU_TL_Defs.rMod[r][g][b]?.tip?.tip?.Replace("\n", " ")}, " +
        ...
        public static void dumpResearchCategories(string dumpFile = "FFU_TL_Research.txt") {
...
namespace CoOpSpRpG {
    public static class patch_LOOTBAG {
        ...
        /// Apply research changes after loading original files.
            orig_prepareLoot();
            if (FFU_TL_Defs.doDataDump) FFU_TL_Research.dumpForgottenResearch("FFU_TL_Forgotten_Original.txt");
            if (FFU_TL_Defs.doDataDump) FFU_TL_Research.dumpResearchCategories("FFU_TL_Research_Original.txt");
            ...
            FFU_TL_Research.rebalanceResearch();
            if (FFU_TL_Defs.doDataDump) FFU_TL_Research.dumpForgottenResearch("FFU_TL_Forgotten_Modded.txt");
            if (FFU_TL_Defs.doDataDump) FFU_TL_Research.dumpResearchCategories("FFU_TL_Research_Modded.txt");
        }
